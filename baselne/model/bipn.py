...
class BIPN(nn.Module):
    ...
    def forward(self, x):
        ...
        p_u_samples, p_i_samples, p_b_samples, gt_samples = torch.chunk(p_samples, 4, dim=-1)
        n_u_samples, n_i_samples, c, gc = torch.chunk(n_samples, 4, dim=-1)
        ...
        bhv_emb = self.bhv_embs[p_b_samples.reshape(-1).long()]
        u_final = self.agg_info(p_u_emb, p_i_emb, bhv_emb) # BCIPN network
        # bc_f_time = time.time() - bc_s_time
        # print(f'BIPN net time : {bc_f_time}')

        # loss calculation
        # l_s_time = time.time()
        log_loss_scores = torch.sum((u_final * p_i_emb), dim=-1).unsqueeze(1)
        ...
        if pair_samples.shape[0] > 0:
            ...
            bpr_score = (1 - lamb) * score_point + lamb * score_gen
            p_score, n_score = torch.chunk(bpr_score, 2, dim=-1)                             # (B, )
